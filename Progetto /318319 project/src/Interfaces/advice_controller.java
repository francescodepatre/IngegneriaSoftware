...
import Operations.buy_proposal;
...
public class advice_controller {
	...
	public void compilebuttonpressed(ActionEvent e) throws IOException{
		for(int i = 0; i < previous_orders.size(); i++) {
			...
			cli.sendData(req);
			*/
			new buy_proposal().prop(previous_orders.get(i).getWine(), previous_orders.get(i).getQuantity(), new Customer_constructor().cutsm_con(4),prop_id);
		}
