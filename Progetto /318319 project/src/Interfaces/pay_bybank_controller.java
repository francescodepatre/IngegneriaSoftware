...
import Operations.Add_quantity;
import Operations.Buy_wine;
import Operations.Customer_constructor;
...
public class pay_bybank_controller {
	...
	public void buybuttonpressed(ActionEvent e) throws IOException{
		for(int i = 0; i < previous_orders.size(); i++) {
			remaining = new Add_quantity().get(previous_orders.get(i).getWine());
			if(remaining < previous_orders.get(i).getQuantity()) {
				prop = 1;
			}
		}
		...
		if(prop > 0) {
			...
			for(int i = 0; i < previous_orders.size(); i ++) {
				...
				cli.sendData(req);
				*/
				new Buy_wine().buy(previous_orders.get(i).getWine(), previous_orders.get(i).getQuantity(), new Customer_constructor().cutsm_con(4), "Bank");
			}
